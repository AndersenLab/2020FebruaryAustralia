...
<!--   dplyr::select(collection_id, collection_type, species_id, pcr_product_its2, worms_on_sample, collection_longitude, collection_latitude) %>% -->
...
<!--   dplyr::mutate(total_collection_n = n()) %>% -->
<!--   dplyr::group_by(collection_type) %>% -->
<!--   dplyr::mutate(total_collection_in_type = n()) %>% -->
<!--   dplyr::arrange(total_collection_in_type) %>% -->
<!--   dplyr::mutate(perc_collection_type =  (total_collection_in_type/total_collection_n)*100) %>% -->
<!--   #dplyr::select(fixed_substrate, plot_class, total_substrates, perc_worm_sub, worm_per_substrate) %>% -->
<!--   dplyr::ungroup() %>% -->
<!--   dplyr::distinct(collection_type, .keep_all = T) %>% -->
<!--   dplyr::mutate(collection_type = factor(collection_type, levels = c("No Nematode", "Tracks only", "Not genotyped", "PCR -", "Other PCR +", "C. briggsae", "C. tribulationis", "C. elegans"))) -->

<!-- # Fig2B plot for rhabditida positive collections -->
<!-- plot_bar_chart <- ggplot(data = bar_chart) + -->
<!--   geom_bar(stat = "identity", aes(x = total_collection_n, y = perc_collection_type, fill = collection_type), colour = "black") + -->
...
<!--   geom_text(aes(x=total_collection_n, y=102, label=paste0("n=",total_collection_n)),  -->
...
target_map
```
<br> <font color="darkorange">*Orange points*</font> are C. tribulationis (16 isolations from 9 collections).
<br> Zoom to see all 9 collections.
...
target_map
```
<br> <font color="darkred">*Red points*</font> are C. elegans (82 isolations from 25 collections).
<br> <font color="darkblue">*Blue points*</font> are C. briggsae (37 isolations from 16 collections).
<br> <font color="darkorange">*Orange points*</font> are C. tribulationis (16 isolations from 9 collections).
<br> Zoom to see all collections.
